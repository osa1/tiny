...
pub struct Input {
    ...
    stdin: tokio::reactor::PollEvented2<mio::unix::EventedFd<'static>>,
}
...
impl Input {
    pub fn new() -> Input {
        ...
        Input {
            ...
            stdin: tokio::reactor::PollEvented2::new(mio::unix::EventedFd(&libc::STDIN_FILENO)),
        }
    }
}

impl futures::Stream for Input {
    type Item = Event;
    ...
    fn poll(&mut self) -> Result<futures::Async<Option<Event>>, tokio::io::Error> {
        ...
        // Try to parse any bytes in the input buffer from the last poll
        {
            ...
            while !buf_slice.is_empty() {
                ...
                match read_fn(buf_slice, &mut self.evs) {
                    Some(buf_slice_) => {
        ...
        if let Some(ev) = self.evs.pop_front() {
            return Ok(futures::Async::Ready(Some(ev)));
        }
        ...
        match self.stdin.poll_read_ready(mio::Ready::readable()) {
            Ok(futures::Async::Ready(_)) => {
                if read_stdin(&mut self.buf) {
                    ...
                    self.stdin.clear_read_ready(mio::Ready::readable()).unwrap();
